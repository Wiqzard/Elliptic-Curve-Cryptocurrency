...
public:
    ...
    explicit CurvePoint(const FieldInt &x_, const FieldInt &y_);
    
    
    // Constructs a normalized point (z=1) from the given string coordinates. Not constant-time.
    explicit CurvePoint(const char *xStr, const char *yStr);
    ...
    void replace(const CurvePoint &other, std::uint32_t enable);
    ...
    // Tests whether this point equals the given point in all 3 coordinates.
    ...
    void toCompressedPoint(std::uint8_t output[33]) const;
